...
static BOOL mon_get_pct(const DWORD i, DWORD *pct) {
  ...
  if (!GetVCPFeatureAndVCPFeatureReply(g_mons[i].hPhysicalMonitor, 0x10, NULL,
                                       &cur, &max))
    return FALSE;
...
static BOOL CALLBACK enum_proc(HMONITOR hm, HDC dc, LPRECT rc, LPARAM lp) {
  ...
  if (GetPhysicalMonitorsFromHMONITOR(hm, cnt, mons)) {
    for (DWORD i = 0; i < cnt && g_mon_cnt < MAX_MON; i++) {
      ...
      if (GetVCPFeatureAndVCPFeatureReply(mons[i].hPhysicalMonitor, 0x10, NULL,
                                          &cur, &max)) {
...
static LRESULT CALLBACK slider_proc(HWND wnd, UINT msg, WPARAM wp, LPARAM lp) {
  ...
  case WM_CREATE:
    ...
    g_label = CreateWindow(L"STATIC", L"", WS_CHILD | WS_VISIBLE | SS_CENTER,
                           10, 50, 280, 20, wnd, NULL, NULL, NULL);

    g_slider =
        CreateWindow(TRACKBAR_CLASS, NULL, WS_CHILD | WS_VISIBLE | TBS_HORZ, 10,
                     80, 280, 30, wnd, NULL, NULL, NULL);
  ...
  case WM_HSCROLL:
    if ((HWND)lp == g_slider) {
      ...
      if (code == TB_ENDTRACK || code == TB_THUMBPOSITION ||
          code == TB_PAGEUP || code == TB_PAGEDOWN || code == TB_LINEUP ||
          code == TB_LINEDOWN) {
        if (pos != g_cur_pct)
          mon_set_pct(pos);
      }
...
static void show_window(void) {
  ...
  g_win = CreateWindowEx(WS_EX_TOOLWINDOW | WS_EX_TOPMOST, L"br_slider", NULL,
...
int WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine,
  ...
  g_main = CreateWindowEx(0, L"br_main", L"", WS_OVERLAPPED, 0, 0, 0, 0, NULL,
                          NULL, hInstance, NULL);
